...
export class AssembledTransaction<T> {
  ...
  get simulationData(): {
    ...
    // TODO: handle simulation restore , bugs out right now
  ...
  signAuthEntries = async (
    ...
    if (!needsNonInvokerSigningBy)
      throw new NoUnsignedNonInvokerAuthEntriesError(
        "No unsigned non-invoker auth entries; maybe you already signed?"
      );
    const publicKey = await this.getPublicKey();
    if (!publicKey)
      throw new Error(
        "Could not get public key from wallet; maybe Freighter is not signed in?"
      );
